...
Cord=["Cord",0]; Theta=["Theta",0]; Length=["Length",0]; Radius=["Radius",0]; Phi=["Phi",0]; Moment=["Moment",0];
cordSwitch =0; thetaSwitch=0; lengthSwitch=0; radiusSwitch=0; phiSwitch=0; momentSwitch=0;
...
{   
    ...
    if (s[0] =="Cord") 
    { 		
        addRemoveObject([xAxis,yAxis,zAxis],cordSwitch,scene);
        cordSwitch = 1-cordSwitch;
    }
    docx.style.setProperty('--x1',cordSwitch);
    if (s[0]=="Theta") 
    { 
        el.innerHTML = '<text>angle is measured from the x axis in anticlockwise direction. Since we are considering only uniform rotation this changes uniformally with the rate ω</text><button onclick="remov()">ok</button>';
        addRemoveObject([thetaSymbol, thetaEllipse],thetaSwitch,scene);
        thetaSwitch = 1-thetaSwitch;
    }
    else if (s[0]=="Phi") 
    { 
        el.innerHTML = '<text> angle is measured from the z. Since we are considering only uniform rotation this remains a constant</text><button onclick="remov()">ok</button>'
        addRemoveObject([phiEllipse],phiSwitch,pivot);
        addRemoveObject([phiSymbol],phiSwitch,scene);
        phiSwitch = 1-phiSwitch;
    }
    else if (s[0]=="Length") 
    { 
        el.innerHTML = '<text> length(r) of the gyroscope is taken as the distance between origin and the center of mass of the gyroscope.</text><button onclick="remov()">ok</button>'
        addRemoveObject([axleLengthIndicator],lengthSwitch,pivot);
        addRemoveObject([axleLengthSymbol],lengthSwitch,scene);
        lengthSwitch = 1-lengthSwitch;
    }
    else if (s[0]=="Radius") 
    { 
        el.innerHTML = '<text> this is the radius(R) of the gyroscope.</text><button onclick="remov()">ok</button>'
        addRemoveObject([diskRadiusIndicator],radiusSwitch,pivot);
        addRemoveObject([diskRadiusSymbol],radiusSwitch,scene);
        radiusSwitch = 1-radiusSwitch;
    }
    else if (s[0]=="Moment") 
    { 
        el.innerHTML = '<text> Moment of inertia(I) gives us an idea of how mass is distributed about the axis of rotation.</text><button onclick="remov()">ok</button>'
        addRemoveObject([momentSymbol],momentSwitch,scene);
        momentSwitch = 1-momentSwitch;
    }
}
...
objectData.position = {x:axleLength-2*fontSize,y:axleLength,z:0};
let axleLengthSymbol = createObject(objectData);
// scene.add(axleLengthSymbol);

objectData.text = "R : " + diskRadius.toFixed(2).toString();
objectData.position = {x:axleLength-2*fontSize,y:axleLength-fontSize*2,z:0};
let diskRadiusSymbol = createObject(objectData);
// scene.add(diskRadiusSymbol);

objectData.text = "I : \u00bdMR\u00b2 \n = \u00bd" + mass.toFixed(2).toString() 
                    + " x " + (diskRadius*diskRadius).toFixed(2).toString() + "\n = " 
                    + I0.toFixed(2).toString();
objectData.position = {x:axleLength-2*fontSize,y:axleLength-fontSize*4,z:0};
let momentSymbol = createObject(objectData);
// scene.add(momentSymbol);
...
sliders[i].slider.addEventListener("input", function(){
    ...
    case "mass":
        ...
        data.text = "I : \u00bdMR\u00b2 \n = \u00bd" + mass.toFixed(2).toString() 
                    + " x " + (diskRadius*diskRadius).toFixed(2).toString() + "\n = " 
                    + I0.toFixed(2).toString();
        momentSymbol.geometry = changeText(data);
        break;
    case "axleLength":
        sliders[i].text.innerHTML = this.value;
